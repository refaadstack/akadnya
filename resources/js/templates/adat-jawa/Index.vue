...
const sortedSections = computed(() => {
    return [...props.invitation.sections].sort(
...
<template>
    <div class="relative min-h-screen bg-amber-50">
        <!-- Background Music -->
        <audio
        ...
        <button
            v-if="invitation.content.music_url"
            @click="toggleMusic"
            class="fixed right-6 bottom-6 z-50 rounded-full bg-amber-800 p-4 text-white shadow-lg transition hover:bg-amber-900"
        >
            <svg
                v-if="isPlaying"
                class="h-6 w-6"
        ...
        <!-- Ornaments - Top -->
        <component
            v-for="ornament in activeOrnaments.filter((o) =>
                o.position.includes('top'),
            )"
            :key="ornament.id"
