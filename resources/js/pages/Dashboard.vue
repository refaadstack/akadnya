...
interface Analytics {
    ...
    rsvp_trend: RsvpTrendDay[];
}

interface RsvpTrendDay {
    date: string;
    label: string;
    total: number;
    attending: number;
...
const attendingShare = (day: RsvpTrendDay): string =>
    day.total > 0 ? `${(day.attending / day.total) * 100}%` : '0%';

const notAttendingShare = (day: RsvpTrendDay): string =>
    day.total > 0
        ? `${((day.total - day.attending) / day.total) * 100}%`
        : '0%';
...
const statCards = computed<StatCard[]>(() => [
    ...
    { label: 'Total Tamu', value: props.stats.total_guests, icon: Users },
    {
        ...
        value: props.stats.confirmed_rsvps,
        icon: CalendarCheck,
    },
...
const quickSteps = [
    {
        title: 'Isi Konten Undangan',
        text: 'Lengkapi data mempelai, acara, dan informasi penting.',
        href: '/dashboard/editor',
        icon: BookOpen,
    },
    {
        title: 'Atur Subdomain',
        text: 'Pilih alamat unik yang mudah dibagikan ke tamu.',
        href: '/dashboard/settings',
        icon: Link2,
    },
    {
        title: 'Kustomisasi Tampilan',
        text: 'Rapikan urutan section, warna, dan ornamen.',
        href: '/dashboard/customize',
        icon: Palette,
    },
    {
        title: 'Publikasikan',
        text: 'Cek preview lalu bagikan undangan saat sudah siap.',
        href: '/dashboard/settings',
        icon: Send,
    },
];
...
const quickActions = [
    {
        ...
        text: 'Pelajari alur membuat undangan dari template sampai publish.',
        icon: BookOpen,
    },
    {
    ...
    {
        title: 'Fitur Premium',
        text: 'Kelola galeri, RSVP, amplop digital, dan detail personal.',
        icon: Sparkles,
    },
];
    ...
    <DashboardLayout>
        ...
        <main class="my-container py-10">
            ...
            >
                <div>
                    ...
                    <p class="my-copy mt-3">
                        Kelola template, konten, tamu, RSVP, dan publikasi
                        undanganmu.
                    </p>
                </div>
            ...
            <div
                ...
                class="mb-6 rounded-lg border border-[var(--my-primary)]/25 bg-[var(--my-primary)]/10 px-4 py-3 font-semibold text-[var(--my-neutral)]"
            >
            ...
            <div
                ...
                class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 font-semibold text-red-700"
            >
                {{ $page.props.flash.error }}
            </div>

            <section
                v-if="invitationOptions.length > 0"
                class="my-card mb-8 p-6"
            >
                <div
                ...
                >
                    <div>
                        ...
                        <p class="mt-1 text-[var(--my-muted)]">
                            Pilih template aktif sebelum mengedit, preview, atau
                            publish undangan.
                        </p>
                    </div>
                    <span class="text-sm font-bold text-[var(--my-primary)]"
                        >{{ invitationOptions.length }} template tersedia</span
                    >
                </div>
                ...
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <button
                        ...
                        :class="
                            option.is_active
                                ? 'border-[var(--my-primary)] bg-[var(--my-primary)]/8'
                                : 'border-[var(--my-border)] bg-white/60'
                        "
                    ...
                    >
                        <div class="flex gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                >
                                    {{ option.template?.slug }}
                                </p>
                                <p
                                    class="mt-2 text-xs font-semibold text-[var(--my-muted)]"
                                >
                                    {{
            ...
            <section v-if="hasInvitation" class="my-card mb-8 p-6">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <button
                        type="button"
                        class="ml-auto text-[var(--my-muted)] transition hover:text-[var(--my-primary)]"
                ...
                >
                    <Link
                        ...
                        :key="step.title"
                        :href="step.href"
                        class="rounded-lg border border-[var(--my-border)] bg-white/55 p-4 transition hover:-translate-y-0.5 hover:border-[var(--my-primary)]"
                    >
                        <component
                            :is="step.icon"
                            class="mb-4 size-6 text-[var(--my-primary)]"
                        />
                        <h3 class="font-bold text-[var(--my-neutral)]">
                            {{ step.title }}
                        </h3>
                        <p
            ...
            <section class="mb-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                ...
                >
                    <component
                        :is="card.icon"
                        class="mb-5 size-7 text-[var(--my-primary)]"
                    />
                    <p class="text-3xl font-bold text-[var(--my-neutral)]">
                        {{ card.value }}
                    </p>
                    <p
                        class="mt-1 text-sm font-semibold text-[var(--my-muted)]"
                    >
                        {{ card.label }}
                    </p>
                </article>
            ...
            <section v-if="hasInvitation" class="my-card mb-8 p-6">
                ...
                >
                    <div>
                        ...
                        <div class="grid gap-2 text-sm text-[var(--my-muted)]">
                            <p>
                                Template:
                                <span
                                    class="font-bold text-[var(--my-neutral)]"
                                    >{{
                    ...
                    <div class="flex flex-wrap gap-3">
                        <Link href="/dashboard/editor" class="my-btn-primary"
                            >Edit Konten</Link
                        >
                        <Link
                            href="/dashboard/customize"
                        ...
                        <button
                            v-if="isPublished"
                            type="button"
                            class="my-btn-secondary border-red-300 text-red-600 hover:bg-red-50"
                        ...
                        <button
                            v-else
                            type="button"
                            class="my-btn-secondary border-[var(--my-primary)] text-[var(--my-primary)]"
                ...
                <article class="my-card p-6">
                    ...
                    <div v-if="recentRsvps.length > 0" class="grid gap-3">
                        <div
                            ...
                            :key="rsvp.id"
                            class="rounded-lg border border-[var(--my-border)] bg-white/55 p-4"
                        >
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <span class="text-xs text-[var(--my-muted)]">{{
                                    rsvp.created_at
                                }}</span>
                            </div>
                            <p
                                v-if="rsvp.message"
                                class="mt-2 text-sm text-[var(--my-muted)]"
                            >
                                {{ rsvp.message }}
                            </p>
                        </div>
                    ...
                    <p
                        v-else
                        class="rounded-lg bg-[var(--my-surface-soft)] px-4 py-8 text-center text-[var(--my-muted)]"
                    >
                        Belum ada RSVP.
                    </p>
                </article>
                ...
                <article class="my-card p-6">
                    ...
                    <div v-if="recentWishes.length > 0" class="grid gap-3">
                        ...
                        >
                            ...
                            >
                                <p class="font-bold text-[var(--my-neutral)]">
                                    {{ wish.name }}
                                </p>
                                <span class="text-xs text-[var(--my-muted)]">{{
                                    wish.created_at
                            ...
                            <p class="text-sm leading-6 text-[var(--my-muted)]">
                                {{ wish.message }}
                            </p>
                        </div>
                    </div>
            ...
            <section v-if="hasInvitation && analytics" class="my-card mb-8 p-6">
                ...
                >
                    <h3 class="my-heading text-2xl">Tren RSVP 14 Hari</h3>
                    <div
                        class="flex flex-wrap items-center gap-4 text-xs text-[var(--my-muted)]"
                    >
                        <span class="flex items-center gap-2">
                            <span
                                class="inline-block h-3 w-3 rounded-sm bg-[var(--my-primary)]"
                            ></span>
                            Hadir
                        </span>
                        <span class="flex items-center gap-2">
                            <span
                                class="inline-block h-3 w-3 rounded-sm bg-[var(--my-primary)]/30"
                            ></span>
                            Tidak hadir
                        </span>
                        <span class="font-bold text-[var(--my-primary)]"
                            >{{ analytics.attendance_rate }}% kehadiran</span
                        >
                        <span
                            v-if="analytics.total_check_ins > 0"
                            class="font-semibold"
                            >{{ analytics.total_check_ins }} check-in</span
                        >
                    </div>
                </div>
                ...
                <div class="flex h-44 items-end gap-1.5 sm:gap-2">
                    <div
                        v-for="day in analytics.rsvp_trend"
                        :key="day.date"
                        class="group flex h-full flex-1 flex-col items-center gap-1"
                    >
                        <span
                            class="text-[11px] font-bold text-[var(--my-neutral)] opacity-0 transition group-hover:opacity-100"
                            >{{ day.total }}</span
                        >
                        <div
                            class="flex w-full flex-1 flex-col justify-end overflow-hidden rounded-t"
                            :title="`${day.date}: ${day.total} RSVP (${day.attending} hadir)`"
                        >
                            <div
                                v-if="day.total === 0"
                                class="h-1 rounded-t bg-[var(--my-surface-soft)]"
                            ></div>
                            <template v-else>
                                <div
                                    class="w-full bg-[var(--my-primary)]"
                                    :style="{ height: attendingShare(day) }"
                                ></div>
                                <div
                                    class="w-full bg-[var(--my-primary)]/30"
                                    :style="{ height: notAttendingShare(day) }"
                                ></div>
                            </template>
                        </div>
                        <span class="text-[10px] text-[var(--my-muted)]">{{
                            day.label
                        }}</span>
                    </div>
                </div>
            ...
            <section v-if="hasInvitation && analytics" class="my-card mb-8 p-6">
                ...
                <div class="grid gap-5 md:grid-cols-3 xl:grid-cols-6">
                    <div class="rounded-lg bg-white/55 p-4 text-center">
                        <BarChart3
                            class="mx-auto mb-3 size-6 text-[var(--my-primary)]"
                        />
                        <p class="text-2xl font-bold text-[var(--my-neutral)]">
                            {{ analytics.total_rsvp }}
                        </p>
                    ...
                    <div class="rounded-lg bg-white/55 p-4 text-center">
                        ...
                        />
                        <p class="text-2xl font-bold text-[var(--my-neutral)]">
                            {{ analytics.total_gallery_photos }}
                        </p>
            ...
            <section
                v-if="!hasInvitation"
                class="my-card px-6 py-14 text-center"
            >
                <Sparkles
                    class="mx-auto mb-5 size-12 text-[var(--my-primary)]"
                />
                <h2 class="my-heading text-3xl">Belum ada undangan</h2>
                <p class="my-copy mx-auto mt-3 max-w-xl">
                    Mulai dari koleksi template, lihat preview aslinya, lalu isi
                    detail acara saat kamu sudah menemukan gaya yang cocok.
                </p>
                <Link href="/templates" class="my-btn-primary mt-8"
                    >Pilih Template</Link
                >
            </section>
            ...
            <section class="mt-8 grid gap-5 md:grid-cols-3">
                ...
                >
                    <component
                        :is="action.icon"
                        class="mb-4 size-7 text-[var(--my-primary)]"
                    />
                    <h3 class="text-lg font-bold text-[var(--my-neutral)]">
                        {{ action.title }}
                    </h3>
                    <p class="mt-2 text-sm leading-6 text-[var(--my-muted)]">
                        {{ action.text }}
                    </p>
                </article>
            </section>
