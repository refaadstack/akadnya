...
test('owner can link an orphan rsvp to a guest from the dashboard', function () {
    $invitation = linkedRsvpInvitation();
    $guest = addGuest($invitation, 'Budi Santoso');
    $rsvp = Rsvp::create([
        'invitation_id' => $invitation->id,
    ...
    $this->actingAs($invitation->user)
        ->post("/dashboard/rsvp/{$rsvp->id}/link", ['guest_id' => $guest->id])
        ->assertRedirect();

    expect($rsvp->fresh()->guest_id)->toBe($guest->id);
    expect($rsvp->fresh()->name)->toBe('Budi Santoso');
});

test('linking to a guest that already has a confirmation is rejected', function () {
    $invitation = linkedRsvpInvitation();
    $guest = addGuest($invitation, 'Budi Santoso');
    Rsvp::create([
        'invitation_id' => $invitation->id,
    ...
    $this->actingAs($invitation->user)
        ->post("/dashboard/rsvp/{$orphan->id}/link", ['guest_id' => $guest->id])
        ->assertStatus(422);

    expect($orphan->fresh()->guest_id)->toBeNull();
});

test('stranger cannot link rsvps on someone elses invitation', function () {
    $invitation = linkedRsvpInvitation();
    $guest = addGuest($invitation, 'Budi Santoso');
    $rsvp = Rsvp::create([
        'invitation_id' => $invitation->id,
    ...
    expect($rsvp->fresh()->guest_id)->toBeNull();
});
